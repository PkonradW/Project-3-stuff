    ...
    while (fgets (buffer, 255, fp) != NULL) {   // this should be in a function
        ...
        if (buffer[0] == '.') {
            ...
            if (strcmp(mneum, start) == 0)  {   // for START only
                ...
                fprintf(rf, "%x   %s", locctr, buffer);
                locctr += 3;
            } else if (strcmp(mneum, byte) == 0) {
                if (isLabel) insert (label, locctr);
                printf("%x   %s\n", locctr, buffer);
                fprintf(rf, "%x   %s", locctr, buffer);
                locctr += biteProcessing(operand); // this literally only works with 3-byte constants
            } else if (strcmp(mneum,end) == 0) {
                printf("%x   %s\n", locctr, buffer);
                fprintf(rf, "%x   %s", locctr, buffer);
...
void optabInit() {
    ...
    */
    while (fgets (buffer, 255, fp) != NULL) {
        ...
        token = strtok(buffer, " ");
        //printf("token: %s\n", token);
        noomonic = malloc(strlen(token) + 1); 
